...
let stats, gui, earth, eclipse, board, loading, sound, message,
    winH, winW, resized, mouseX, mouseY
let $win = $(window)
let $doc = $(document)
let $body = $('body')
...
const amount = isMobile ? 700 : 1600
const ringSize = 230
const appW = 2016
const appH = 864
const boardW = 800
const boardH = 600
const ratio = appW / appH
...
function createGui() {
    ...
    for (let i in delta) {
        let f = gui.addFolder(i)
        let obj = delta[i]
        f.add(obj.alpha, 'from', 0, 1).name('alpha from').step(0.1)
        f.add(obj.alpha, 'to', 0, 1).name('alpha to').step(0.1)
        f.add(obj.speed, 'go', 0, 3).name('speed go').step(0.1)
        f.add(obj.speed, 'back', 0, 3).name('speed back').step(0.1)
        f.add(obj.delay, 'go', 0, 3).name('delay go').step(0.1)
        f.add(obj.delay, 'back', 0, 3).name('delay back').step(0.1)
    }

    let f = gui.addFolder('Board')
...
class Eclipse {
    ...
    show() {
        ...
        for (let i = 0; i < earth.stars.length; i++) {
            this.tweens.push(TweenMax.to(earth.stars[i], 3, {
                scaleDelta: .7
            }))
        }
    }
    hide() {
        ...
        for (let i = 0; i < earth.stars.length; i++) {
            this.tweens.push(TweenMax.to(earth.stars[i], 1, {
                scaleDelta: .3
            }))
        }
    }
...
class Loading {
    ...
    listenEvent() {
        PubSub.subscribe('resize', (msg, data) => {
            this.$loading.css({
                transform: data.css
            })
        });
...
class Earth {
    ...
    initFront() {
        ...
        for (i = 0; i < 34; i++) {
            let texture = PIXI.Texture.fromFrame('body_' + (i + 1) + '.png')
        ...
        for (i = 0; i < 7; i++) {
            let texture = PIXI.Texture.fromFrame('headglasses_' + (i + 1) + '.png')
            this.headSprites.push(texture)
        }
        ...
        if (!isMobile) {
            this.moonCtrl.on("pointerover", () => {
                eclipse.show()
                this.$front.addClass('over')
            })

            this.moonCtrl.on("pointerout", () => {
                this.$front.removeClass('over')
                eclipse.hide()
            })

            this.moonCtrl.on("pointerdown", () => {
                // board.isShow = true
                // board.update()
                board.open()
            })
        } else {
            this.moonCtrl.on("pointerdown", (e) => {
                eclipse.show()
                board.open()
            })
        }
    ...
    initBack() {
        ...
        // nebula
        this.nebula = new PIXI.Sprite(
            PIXI.loader.resources[assets.nebula].texture
        )
        ...
        this.back.ticker.add((deltaTime) => {
            hasStats && stats.begin()
            this.fps = 60 / deltaTime
            this.loop()
            hasStats && stats.end()
        })
    }
    ...
    listenEvent() {
        PubSub.subscribe('resize', (msg, data) => {
            this.front.renderer.view.style.transform = data.css
            this.back.renderer.view.style.transform = data.css
        });
    }
}
...
void main() {
    ...
    slide() {
        ...
        TweenMax.to(this.time, this.duration.value, {
            ...
            onUpdate: () => {
                this.uniforms.time.value = this.time.value
            },
            onComplete: () => {
                this.playing = false
            }
    ...
    listenEvent() {
        PubSub.subscribe('resize', (msg, data) => {
            ...
            if (winH > winW) {
                this.camera.position.z = 46 + (winH / winW * 10)
            } else {
                this.camera.position.z = 46;
            }
        ...
        document.body.addEventListener('mousemove', (e) => {
            ...
            if (!this.playing && intersects.length > 0) {
            ...
            ry = (e.x / this.renderer.getSize().width - 0.5) * delta
            rx = (e.y / this.renderer.getSize().height - 0.5) * delta
            TweenMax.to(this.group.rotation, 0.5, {
                'x': rx,
                'y': ry
            })
        })
    ...
    bindEvent() {
        this.$volume.on('click', (e) => {
            if (this.playing) {
                ...
                this.playing = true
                this.$volume.removeClass('stopped')
            }
        })
    }
}
...
class Message {
    constructor() {
        ...
        this.$msg.find('.message-close').on('click', (e) => {
            this.$msg.hide()
        })
    }
    slow(averFps) {
